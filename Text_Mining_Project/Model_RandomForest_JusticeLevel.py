...
data = pandas.read_csv("/home/ahlem/Text_Mining_Project/preprocessed_data_justiceLevel.csv", encoding="ISO-8859-1").astype('int')
...
for term in term_range:
    ...
    target_data_train = data.loc[train_index, "justice_level_decision"]
    ...
    feature_data_test = data.loc[test_index, ['term', 'natural_court', 'argument_month', 'decision_month',
                   ...
                   'issue', 'issueArea', 'lawType', 'lawSupp', 'petitionerState', 'respondentState',
                   'partyWinning', 'majVotes']]
    target_data_test = data.loc[test_index, "justice_level_decision"]
    ...
    data.loc[test_index, "rf_predicted"] = m.predict(feature_data_test)
    current_term_data = data.loc[data["term"] == term]
    count_0 = len(current_term_data.loc[current_term_data["rf_predicted"] == 0].index)
    count_1 = len(current_term_data.loc[current_term_data["rf_predicted"] == 1].index)
    count_other = len(current_term_data.loc[current_term_data["rf_predicted"] == -1].index)
...
target_actual = data.loc[evaluation_index, "justice_level_decision"]
target_predicted = data.loc[evaluation_index, "rf_predicted"]


# Evaluate the model
print("Random forest  model evaluation metrics")
print("classification Report","="*32)
print(sklearn.metrics.classification_report(target_actual, target_predicted))
print("Confusion Matrix","="*40)
